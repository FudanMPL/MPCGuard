...
def detect_leakage(secret):
    logger.info("detect leakage for output with secret: {}".format(secret))
    ...
    end_time = time.time()
    config.times['vul_verify'] += end_time - start_time


    logger.info(f" accuracy_real: {accuracy_real} accuracy_ideal: {accuracy_ideal}")
    if accuracy_real > accuracy_ideal + config.accuracy_verify_gap:
        start_time = time.time()
        with open(config.bug_file, "a") as f:
            f.write("\n")
        ...
        end_time = time.time()
        config.times['vul_location'] += end_time - start_time  
        return True
