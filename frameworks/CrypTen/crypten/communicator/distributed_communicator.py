...
class DistributedCommunicator(Communicator):
    ...
    def recv(self, tensor, src=None):
        ...
        my_print_tool.my_print_or_match(result.data, "recv")
        return result
    ...
    def irecv(self, tensor, src=None):
        """Receives a tensor from an (optional) source src."""
        assert dist.is_initialized(), "initialize the communicator first"
        result =  dist.irecv(tensor.data, src=src, group=self.main_group)
        return result
    ...
    def reduce(self, input, dst, op=ReduceOp.SUM, batched=False):
        ...
        if dst != self.get_rank():
            result = None 
        import my_print_tool
        my_print_tool.my_print_or_match(result, "reduce")
        return result
    ...
    def broadcast(self, input, src, group=None, batched=False):
        ...
        if batched:
            ...
            for req in reqs:
                req.wait()
            my_print_tool.my_print_or_match(tensor.data, "broadcast1")
        else:
            assert torch.is_tensor(
                input.data
            ), "unbatched input for reduce must be a torch tensor"
            dist.broadcast(input.data, src, group=group)
            my_print_tool.my_print_or_match(input.data, "broadcast2")
        return input
