...
if __name__ == '__main__':
    command = ['python', './compile.py', '-R', '64', 'eq']  
    ...
    folder_path = "./real-data/rss_eq/s-"+str(args.secret)+"/"
    if args.no_record:
    ...
    for i in tqdm(range(args.try_times)):
        ...
        # execute ./replicated-ring-party.x 0 eq -OF . -pn 14752 -h localhost
        command0 = ["./replicated-ring-party.x", "0", "eq", "-OF", ".", "-pn", "14752", "-h", "localhost",  "-b", "1", "-B", "3"]
        command1 = ["./replicated-ring-party.x", "1", "eq", "-OF", ".", "-pn", "14752", "-h", "localhost",  "-b", "1", "-B", "3"]
        command2 = ["./replicated-ring-party.x", "2", "eq", "-OF", ".", "-pn", "14752", "-h", "localhost",  "-b", "1", "-B", "3"]
