...
if __name__ == "__main__":
    ...
    my_print_tool.folder_path = "real-data/pond_linear/s-"+str(args.secret)+"/"
    ...
    secret = args.secret
    # set tfe protocol
    prot = globals()['Pond'](fixedpoint_config=my_fixed)
    ...
    def test(secret1, secret2=1):
        ...
        factory = prot.tensor_factory
        with tf.device(prot.server_0.device_name):
            y.share0 =factory.tensor(1)
        with tf.device(prot.server_1.device_name):
            y.shares1 =factory.tensor(1)
        ...
        with tf.device(prot.server_1.device_name):
            my_print_tool.my_print_or_match(result.share1.value, "output")
