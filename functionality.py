...
def generate_ass_shares(secret):
    ...
    shares = []
    from config import number_of_party
    for i in range(number_of_party - 1):
...
def generate_rss_shares(secret):
    assert config.number_of_party == 3
    secret = np.array(secret, dtype=np.int64)
    # Generate random shares of a secret
    shares = []
    from config import number_of_party
...
# def F_ass_linear(shares_x, shares_y=None, a=1, b=2, c=3):
#     # Convert constants and reconstruct x and y from shares using numpy arrays
#     a = np.array(a, dtype=np.int64)
#     b = np.array(b, dtype=np.int64)
#     c = np.array(c, dtype=np.int64)
...
#     return generate_ass_shares(z)


# def F_ass_mul(shares_x, shares_y=None):
#     # Reconstruct x and y using numpy arrays
#     x = sum(np.array(shares_x, dtype=np.int64))
...
# def F_ass_truncpr(shares_x, f=10):
#     # Reconstruct x
#     x = sum(np.array(shares_x, dtype=np.int64))
...
# def F_ass_share(shares_x):
#     x = sum(np.array(shares_x, dtype=np.int64))
#     return generate_ass_shares(x)


# def F_ass_shuffle(shares_x, shares_y=None):
#     # Reconstruct x and y using numpy arrays
#     x = sum(np.array(shares_x, dtype=np.int64))

#     if shares_y is None:
#         shares_y = [0 for i in range(len(shares_x))]
#     y = sum(np.array(shares_y, dtype=np.int64))
#     r = random.random()
#     if r < 0.5:
#         return generate_ass_shares(x)
#     else:
#         return generate_ass_shares(y)


# def F_ass_and(shares_x, shares_y=None):
#     # Reconstruct x and y using numpy arrays
#     x = sum(np.array(shares_x, dtype=np.int64))
